    ...
    public class SolutionsFinder : IFindSolutions
    {
        public List<Solution> FindAllSolutions(List<int> numbers, List<Operator> operators)
        {
           ...
           // var operatorsCombinations = GetCombinations(operators, numbers.Count - 1);
            ...
            foreach (var operatorsVariation in operatorsVariations)
            {
                var solution = new Solution(numbersPermutation, operatorsVariation);
        ...
        private List<List<T>> GetVariations<T>(List<T> list, int length)
        {
            var permutations = new Variations<T>(list, length, GenerateOption.WithRepetition);
        ...
        {
            var combinations = new Combinations<T>(list, length, GenerateOption.WithRepetition);
            return combinations.Select(permutation => permutation.ToList()).ToList();
        }
